...
C_CSPlayerPawn* CCSPlayerController::GetPlayerPawn() {
	CBaseHandle hPlayerPawn = this->GetPlayerPawnHandle();

	if (!hPlayerPawn.IsValid())
		return nullptr;

	int nIndex = hPlayerPawn.GetEntryIndex();
	return I::GameResourceService->pGameEntitySystem->Get<C_CSPlayerPawn>(nIndex);
}

C_CSWeaponBase* C_CSPlayerPawn::GetActiveWeapon() {
	CCSPlayer_WeaponServices* pWeaponServices = GetWeaponServices();
	...
	int nIndex = hActiveWeapon.GetEntryIndex();
	C_CSWeaponBase* pActiveWeapon = I::GameResourceService->pGameEntitySystem->Get<C_CSWeaponBase>(nIndex);
...
bool C_CSPlayerPawn::HasArmour(const int iHitgroup) {
