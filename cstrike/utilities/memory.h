...
class CAddress {
protected:
	mutable std::uint8_t* nAddressBytes;

public:
	CAddress() = default;
	CAddress(std::uint8_t* value) : nAddressBytes(value) {};

public:
	bool operator==(const CAddress& pAddress) const {
		return nAddressBytes == pAddress.nAddressBytes;
	}

public:
	template<typename T>
	T as() {
		return reinterpret_cast<T>(nAddressBytes);
	}
	...
	std::uint8_t* get() {
		return nAddressBytes;
	}

	CAddress add(int iOffset) {
		nAddressBytes += iOffset;

		return *this;
	}

	CAddress abs(int nPreOffset = 0x1, int nPostOffset = 0x0) {
		nAddressBytes += nPreOffset;
		nAddressBytes += sizeof(std::int32_t) + *reinterpret_cast<std::int32_t*>(nAddressBytes);
		nAddressBytes += nPostOffset;

		return *this;
	}

	CAddress rel(std::uint32_t nRVAOffset = 0x3) {
		std::uint32_t nRIPOffset = nRVAOffset + sizeof(int);
		...
		nAddressBytes = reinterpret_cast<std::uint8_t*>(nRVA + nRIP);
...
namespace MEM
{
	...
	/* @section: search */
	[[nodiscard]] CAddress FindPattern2(const wchar_t* wszModuleName, const char* szPattern);
