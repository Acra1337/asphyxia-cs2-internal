...
std::string GetWindowsFontPath(const std::string& fontName) {
	char systemRoot[MAX_PATH];
	if (GetEnvironmentVariableA("SystemRoot", systemRoot, MAX_PATH)) {
		return std::string(systemRoot) + "\\Fonts\\" + fontName;
	}
	return "C:\\Windows\\Fonts\\" + fontName; // Фолбэк на стандартный путь
}
...
#pragma endregion
	...
	for (int i = 0; i < CS_ARRAYSIZE(FONT::pMenu); i++)
	{
		...
		FONT::pMenu[i] = io.Fonts->AddFontFromFileTTF(CS_XOR(get_font_path1.c_str()), flFontSize, &imVerdanaConfig, io.Fonts->GetGlyphRangesCyrillic());
